...
const App = ({ Component, pageProps }: AppProps) => {
  ...
  useEffect(() => {
    const fetchWebsiteConfig = async () => {
      const language =
        Cookies.get("language") || process.env.NEXT_PUBLIC_LOCALE;
  ...
  useEffect(() => {
    const handleRouteChange = (url: string) => {
      const route = url.split("?")[0].slice(1) || "home"; // Remove leading slash and set 'home' for root
      document.body.className = route; // Update body class
    };
